...
std::ostream& operator<<(std::ostream& s, decimal d)
{
	...
	ss << std::dec << minus 
		<< static_cast<long long int>(d.value()/DECIMAL_PRECISION)
		<< "."
		<< std::setw(6) << std::setfill('0')
		<< static_cast<long long int>((d.value() > 0 ? d.value() : -d.value())%DECIMAL_PRECISION);

	//s << boost::trim_right_copy_if(ss.str(), boost::is_any_of("0"));
	s << boost::trim_right_copy_if(ss.str(), std::bind2nd(std::equal_to<char>(), '0'));
	return s;
}
