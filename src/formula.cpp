...
namespace game_logic
{
	...
	variant formula_callable::get_value_by_slot(int slot) const
	{
		...
		std::cerr << "Could not get value by slot from formula callable " << slot << std::endl;
		return variant(); //so VC++ doesn't complain
	}
...
private:
	variant execute(const formula_callable& variables) const {
		const variant left = left_->evaluate(variables);
		size_t begin_index = start_ ? start_->evaluate(variables).as_int() : 0;
		size_t end_index = end_ ? end_->evaluate(variables).as_int() : left.num_elements();
...
private:
	...
	variant get_value_by_slot(size_t slot) const {
...
void formula::check_brackets_match(const std::vector<token>& tokens) const
{
	...
	if(error_loc != -1) {
		...
		if(whitespace.size() > 40) {
			const size_t erase_size = whitespace.size() - 60;
