...
#define round boost::math::tr1::round
#endif
...
namespace {
	...
	private:
		variant execute(const formula_callable& variables) const {
			const size_t i = args()[0]->evaluate(variables).as_bool() ? 1 : 2;
			if(i >= args().size()) {
				return variant();
			}
...
FUNCTION_DEF(sin, 1, 1, "sin(x): Standard sine function.")
	const float angle = float(args()[0]->evaluate(variables).as_decimal().as_float());
...
FUNCTION_DEF(sqrt, 1, 1, "sqrt(x): Returns the square root of x.")
	const double value = float(args()[0]->evaluate(variables).as_decimal().as_float());
...
FUNCTION_DEF(angle, 4, 4, "angle(x1, y1, x2, y2) -> int: Returns the angle, from 0°, made by the line described by the two points (x1, y1) and (x2, y2).")
	const float a = float(args()[0]->evaluate(variables).as_int());
	const float b = float(args()[1]->evaluate(variables).as_int());
	const float c = float(args()[2]->evaluate(variables).as_int());
	const float d = float(args()[3]->evaluate(variables).as_int());
...
FUNCTION_DEF(orbit, 4, 4, "orbit(x, y, angle, dist) -> [x,y]: Returns the point as a list containing an x/y pair which is dist away from the point as defined by x and y passed in, at the angle passed in.")
	const float x = float(args()[0]->evaluate(variables).as_decimal().as_float());
	const float y = float(args()[1]->evaluate(variables).as_decimal().as_float());
	const float ang = float(args()[2]->evaluate(variables).as_decimal().as_float());
	const float dist = float(args()[3]->evaluate(variables).as_decimal().as_float());
...
FUNCTION_DEF(fold, 2, 3, "fold(list, expr, [default]) -> value")
	...
	a = list[0];
	for(size_t n = 1; n < list.num_elements(); ++n) {
		b = list[n];
...
FUNCTION_DEF(index, 2, 2, "index(list, value) -> index of value in list: Returns the index of the value in the list or -1 if value wasn't found in the list.")
	...
	for(size_t n = 0; n < li.num_elements(); n++) {
		if(value == li[n]) {
			return variant(n);
		}
	}
...
namespace {
void evaluate_expr_for_benchmark(const formula_expression* expr, const formula_callable* variables, int64_t ntimes)
{
	for(int64_t n = 0; n < ntimes; ++n) {
	...
	private:
		variant execute(const formula_callable& variables) const {
			...
			if(args().size() >= 2) {
				...
				boost::split(chopped, thestring, boost::is_any_of(delimiter));
			} else {
				const std::string& thestring = args()[0]->evaluate(variables).as_string();
				boost::split(chopped, thestring, std::bind2nd(std::equal_to<char>(), '\n'));
			}
	...
	formula_function_expression_ptr formula_function::generate_function_expression(const std::vector<expression_ptr>& args_input) const
	{
		...
		if(args.size() + default_args_.size() >= args_.size()) {
			...
			while(args.size() < args_.size()) {
				const size_t index = args.size() - base;
...
UNIT_TEST(sqrt_function) {
	...
	for(uint64_t n = 0; n < 100000; n += 1000) {
		CHECK_EQ(game_logic::formula(variant(formatter() << "sqrt(" << n << ".0^2)")).execute().as_decimal(), decimal::from_int(int(n)));
	}
}
