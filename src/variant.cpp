...
variant variant::get_list_slice(size_t begin, size_t end) const
{
...
variant variant::operator()(const std::vector<variant>& args) const
{
	...
	const size_t max_args = fn_->end_args - fn_->begin_args;
	const size_t min_args = max_args - fn_->default_args.size();
...
variant* variant::get_index_mutable(size_t index)
{
	if(is_list()) {
...
void variant::serialize_to_string(std::string& str) const
{
	...
	case TYPE_FUNCTION:
		std::cerr << "ATTEMPT TO SERIALIZE FUNCTION: " << fn_->fn->str().c_str() << std::endl;
		assert(false);
	default:
		assert(false);
	}
...
std::string variant::to_debug_string(std::vector<const game_logic::formula_callable*>* seen) const
{
	...
	case TYPE_CALLABLE_LOADING: {
		std::ostringstream ss;
		ss << "(loading " << std::hex << callable_loading_ << ")";
		s << ss.str();
	}
